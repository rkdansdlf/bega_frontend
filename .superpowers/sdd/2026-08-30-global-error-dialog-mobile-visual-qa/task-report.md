...
- Feedback timeout evidence no longer reads the callback counter as soon as the busy DOM appears. The test explicitly polls until the injected submitter count is `1`, then waits exactly two animation frames and reasserts submitter `1`, retry/fetch/XHR/sendBeacon/aggregate network `0`, page errors `0`, and the still-pending busy/disabled UI. `GLOBAL_ERROR_DIALOG_MUTATE_FEEDBACK_EARLY_READ=1` keeps the test-only timeout handoff pending and removes the counter wait; the focused actual-browser test deterministically REDs `0/1` with actual `0` versus expected `1`, while the normal test is GREEN `1/1`. No product source changes are involved.
...
- After the feedback race fix, detached clean checkout `/private/tmp/global-error-race-clean-12f161fa` passed the full actual suite `8/8` twice consecutively, the feedback-focused test `1/1` twice consecutively, and the stale-retry-focused test `1/1`. TypeScript, adapter/catalog/semantic `267/267`, coordinator `12/12`, and the production build with all `153/153` budgets also passed. The final amended commit differs from that verified source only in this documentation of the results.
