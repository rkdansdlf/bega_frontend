...
- Explicit `min-h-11`/`min-w-11` and mobile text sizing live in the owned Offseason files, including `size="sm"` row actions and formerly `h-10` selects. The header close button remains rendered by `PlainDialog`, but its 44px sizing and Visual QA touch audit are now independently enforced by exact selectors in the owned Offseason dialog module rather than inherited shared-primitive styling.
...
- Final independent-review evidence replaced every prior PNG, including the rejected 43/44 select-key run: 44/44 pass, failed 0, recovered 0, all attempt 1, 44 unique IDs, paths, and SHA-256 values, 44 nonzero PNG files, missing/stale 0. All 20 interaction records are capture-verified after viewport expansion. The team fixture starts at `ALL` / `팀 전체`; captured change ends at `LG` / `LG 트윈스` while the focus capture remains `ALL` / `팀 전체`, and their current final-evidence SHA values are distinct (`8b79f07f...` vs `2bb7b7d0...`). Reviewer pairs are also distinct in the current final evidence: hover edit `85ccfa8b...` vs delete `d9dac8c8...`, and pressed apply `1fa771be...` vs reset `d66a39b4...`; each report record also verifies the final pseudo/DOM state after expansion.
...
## Final close-control proof

- Owned-source RED was 8 pass / 1 intended failure for the absent scoped close-size and touch-audit guarantee.
- `OffseasonMovementAdminDialogs` now scopes `min-width: 44px` and `min-height: 44px` only to `[aria-label="닫기"]` inside `admin-offseason-dialog` and `admin-offseason-delete-dialog`, and marks those rendered buttons with `data-vqa-min-touch="44"` for the existing harness audit.
- Real 320x844 DOM measurements are create `44x44`, edit `44x44`, and delete `44x44`; all three are enabled/focusable and retain the accessible close name.
- Shared `plain-dialog.tsx` and `plain-button.tsx` were neither edited nor committed. Their worktree/index hashes remained `7797fcaf...` / `f3ee5fd7...` and `e08b9780...` / `d3ea5534...`, respectively.
- After a 1/1 outside-evidence dialog warm-up, all prior evidence was discarded. The final report is 44/44, failed 0, recovered 0, all attempt 1, with 44 unique IDs, paths, PNGs, and SHA-256 values and all 20 interactions capture-verified after expansion.
- Final scoped GREEN is 319/319. TypeScript, 5,733-module production build, 153 budgets, CSS 255,429/255,500B, root 13,157/18,000B, content 12,585/25,000B, production isolation/import graph 4/4, DOM probe, policy, and diff checks pass. Catalog totals and exact three hosted bindings are unchanged.
