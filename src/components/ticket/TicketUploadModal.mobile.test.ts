...
test('TicketUploadModal exposes deterministic states and mobile-safe upload controls', async () => {
  ...
  assert.match(source, /const mobileBodyScrollStyle = \{ scrollPaddingBlock: '96px' \} as const;/);
  ...
  assert.match(source, /flex flex-col gap-6 pb-1 pt-1/);
  assert.match(source, /style=\{\{ minHeight: 176, padding: 24 \}\}/);
  assert.match(source, /break-keep text-body/);
